...
# Chapter 9: Review Questions - Attempt 4
...
### My answer:
* The path will be `/user/.././root/../kodiacbear.txt`
* So the answer is A
* **A**❌❌❌❌
* **CORRECT ANSWER: F**
* There is no `.get()` method on `Path` class, only `Paths`
* Also the `Paths.get()` method takes a Path not a String!
* If these compiler errors were resolved,then it would be A!!!
<br>
...
* E - false
* F - false
* I'm not sure if the code DOES compile
* But lets assume it does
* **C,D**❌❌❌❌
* **CORRECT ANSWER: B,C**
* The `Files.isDirectory()` AND `Files.deleteIfExists()` methods do support symbolic links!! so B is correct
* D is not correct as it cannot deleted a non-empty directory!!!
<hr>
...
* **E**❌❌❌❌
* **CORRECT ANSWER: D**
* There is no `setTimes()` method for BasicFileAttributes!
<hr>
...
### My answer:
* The code does not compile I think
* **G**❌❌❌❌
* **CORRECT ANSWER: C**
* `path.subpath(1,3).getName(1)` returns `bear`
* `bear` toAbsolutePath returns `/user/home/bear`
<hr> 
...
### My answer:
* second one will be `/pets/../cat.txt`, so it's A
* **A**✅✅✅✅
<br>
...
* **A,B,C,E**❌❌❌❌
* **CORRECT ANSWER: B,C**
* While A is true, this question is asking why prefer onne of the other
* E is not relevant to either methods
<hr>
...
* **A,C,E**❌❌❌❌
* **CORRECT AMSWER: C,E**
* Since `NO_FOLLOWLINKS` is provided
* A WOULD be true if `REPLACE_EXISTING` was specified
* B is false since `NO_FOLLOWLINKS` is provided.
...
### My answer:
* **A**✅✅✅✅
* The files are same in content but it is not pointed to the same file!!!
<br>
...
* The max depth is set to 0, so it only looks at top level directory - so nothing is printed!!s
<hr>
...
* NIO.2 DOES support symbolic links
<br>
